...
import com.dog.cloud.gateway.config.properties.IgnoreWhiteProperties;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String userStr = redisUtils.get(getTokenKey(token));
        if (StringUtils.isNull(userStr)) {
            return setUnauthorizedResponse(exchange, "登录状态已过期");
        }
        JSONObject obj = JSONObject.parseObject(userStr);
        String userid = obj.getString(CacheConstants.DETAILS_USER_ID);
        String username = ServletUtils.urlEncode(obj.getString(CacheConstants.DETAILS_USERNAME));
        if (StringUtils.isBlank(userid) || StringUtils.isBlank(username)) {
            return setUnauthorizedResponse(exchange, "令牌验证失败");
        }

        // 设置过期时间
        redisUtils.expire(getTokenKey(token), EXPIRE_TIME);
        // 设置用户信息到请求
