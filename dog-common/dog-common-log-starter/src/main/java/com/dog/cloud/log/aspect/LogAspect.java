package com.dog.cloud.log.aspect;
...
import com.dog.cloud.log.enums.BusinessStatus;
import com.dog.cloud.log.service.AsyncLogService;
...
public class LogAspect {
    ...
    // 配置织入点
    @Pointcut("@annotation(com.dog.cloud.log.annotation.Log)")
        ...
        if (paramsArray != null && paramsArray.length > 0) {
            for (int i = 0; i < paramsArray.length; i++) {
                if (!isFilterObject(paramsArray[i])) {
                    try {
                        Object jsonObj = new Gson().toJson(paramsArray[i]);
